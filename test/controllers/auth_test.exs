defmodule Rub.AuthTest do
  ...
  setup %{conn: conn} do
    conn =
      conn
      |> bypass_through(Rubl.Router, :browser)
      |> get("/")
  ...
  test "call places user from session in to assigns", %{conn: conn} do
    user = insert_user()
    conn =
      conn
